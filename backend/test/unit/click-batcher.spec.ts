...
describe('ClickBatcher', () => {
  ...
  it('flushes immediately when the buffer reaches BATCH_SIZE', async () => {
    ...
    const batcher = new ClickBatcher<number>(2, 10_000, async (batch) => {
      batches.push(batch);
    });
    await batcher.add(1);
  ...
  it('flushes after BATCH_INTERVAL_MS when below the size threshold', async () => {
    jest.useFakeTimers();
    const batches: number[][] = [];
    const batcher = new ClickBatcher<number>(100, 1_000, async (batch) => {
      batches.push(batch);
    });
    await batcher.add(7);
    expect(batches).toHaveLength(0);
  ...
  it('manual flush is a no-op on an empty buffer', async () => {
    let calls = 0;
    const batcher = new ClickBatcher<number>(10, 1_000, async () => {
      calls += 1;
    });
    await batcher.flush();
    expect(calls).toBe(0);
  ...
  it('does not propagate a handler rejection from the timer callback', async () => {
    jest.useFakeTimers();
    const batcher = new ClickBatcher<number>(100, 500, async () => {
